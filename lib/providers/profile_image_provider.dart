...
class ProfileImageProvider with ChangeNotifier {
  ...
  void startListeningToProfileImages() {
    ...
    _imageSubscription = FirebaseFirestore.instance.collection('requests').snapshots().listen(
          (querySnapshot) {
        ...
        _isLoading = false;
        debugPrint("✅ IMAGE PROVIDER: Requests Collection එකෙන් පින්තූර ${_profileImages.length} ක් අප්ඩේට් වුණා!");
        notifyListeners();
      },
      onError: (error) {
