...
class AdminTripsProvider with ChangeNotifier {
  ...
  // Filtered lists
  List<AdminTripModel> get completedTrips => _trips.where((t) => t.status == 'completed').toList();
  List<AdminTripModel> get cancelledTrips => _trips.where((t) => t.status == 'cancelled').toList();
  List<AdminTripModel> get bookingTrips => _trips.where((t) => t.type == 'booking').toList();
  List<AdminTripModel> get pickupTrips => _trips.where((t) => t.type == 'road_pickup').toList();

  // Summary Metrics
  double get totalBookingsRevenue => bookingTrips.where((t) => t.status == 'completed').fold(0, (total, trip) => total + trip.finalFare);
  double get totalPickupsRevenue => pickupTrips.where((t) => t.status == 'completed').fold(0, (total, trip) => total + trip.finalFare);
  int get totalCompletedCount => completedTrips.length;
  int get totalCancelledCount => cancelledTrips.length;

  Future<void> fetchTripsForDateRange(DateTime startDate, DateTime endDate) async {
    _isLoading = true;
    _trips = [];
    notifyListeners();
    ...
    try {
      ...
      for (var doc in bookingsSnap.docs) {
        ...
        if (status == 'completed' || tripState == 'completed' || paymentStatus == 'collected') {
          finalStatus = 'completed';
        } else if (status == 'cancelled' || tripState == 'cancelled' || status == 'rejected') {
          finalStatus = 'cancelled';
        }

        DateTime tripDate;
        if (data['pickupTime'] != null) {
          tripDate = DateTime.tryParse(data['pickupTime'].toString()) ?? startOfDay;
        ...
        if (data['createdAt'] != null && data['createdAt'] is Timestamp) bookingTime = (data['createdAt'] as Timestamp).toDate();
        if (bookingTime == null && data['timestamp'] != null && data['timestamp'] is Timestamp) bookingTime = (data['timestamp'] as Timestamp).toDate();
        if (data['arrivedAt'] != null && data['arrivedAt'] is Timestamp) driverReachedTime = (data['arrivedAt'] as Timestamp).toDate();
        if (data['startedAt'] != null && data['startedAt'] is Timestamp) tripStartTime = (data['startedAt'] as Timestamp).toDate();
        if (data['completedAt'] != null && data['completedAt'] is Timestamp) tripEndTime = (data['completedAt'] as Timestamp).toDate();

        double estimatedFare = double.tryParse(data['estimateFare']?.toString() ?? '0') ?? 0;
        double finalFare = double.tryParse(data['totalFare']?.toString() ?? '0') ?? 0;
        if (finalFare == 0) finalFare = double.tryParse(data['bidAmount']?.toString() ?? '0') ?? 0;

        String pickupLoc = 'Unknown Pickup';
        if (data['pickupLocation'] is Map) {
          pickupLoc = data['pickupLocation']['address']?.toString() ?? 'Unknown Pickup';
        ...
        if (data['dropLocation'] is Map) {
          dropLoc = data['dropLocation']['address']?.toString() ?? 'Unknown Drop';
        } else if (data['dropLocation'] != null) {
          dropLoc = data['dropLocation'].toString();
        }

        fetchedTrips.add(AdminTripModel(
          id: doc.id,
          type: 'booking',
          date: tripDate,
          estimatedFare: estimatedFare,
          finalFare: finalFare,
          distanceKm: double.tryParse(data['distance']?.toString() ?? data['distanceKm']?.toString() ?? '0') ?? 0,
          waitingTimeSec: int.tryParse(data['waitingTimeSec']?.toString() ?? data['waitingTime']?.toString() ?? '0') ?? 0,
          ...
          passengerPhone: data['memberPhone']?.toString() ?? data['passengerPhone']?.toString() ?? 'Unknown',
          ...
          pickupFutures.add(_firestore
            ...
            for (var doc in snap.docs) {
              ...
              if (data.containsKey('tripId')) {
                 ...
                 String status = 'completed'; // Road pickups are generally recorded only when complete
                 ...
                 fetchedTrips.add(AdminTripModel(
                   ...
                   type: 'road_pickup',
                   date: tripDate,
                   estimatedFare: finalFare, // Road pickups don't have an estimate
                   finalFare: finalFare,
                   distanceKm: double.tryParse(data['distanceKm']?.toString() ?? '0') ?? 0,
                   waitingTimeSec: int.tryParse(data['waitingTimeSec']?.toString() ?? '0') ?? 0,
                   startAddress: data['startAddress'] ?? 'Street Pickup',
                   endAddress: data['endAddress'] ?? 'Street Drop',
                   driverName: 'Driver $membershipNo', // Meter provider often doesn't save name, so we use ID
                   ...
                   tripEndTime: status == 'completed' ? tripDate.add(Duration(minutes: (finalFare / 100).round())) : null,
                   status: status,
                   paymentMethod: 'Cash',
                   docId: doc.id,
                 ));
  ...
  Future<bool> deleteTrip(AdminTripModel trip) async {
    try {
      if (trip.type == 'booking') {
        ...
        if (trip.driverMembershipNo != '-' && trip.driverMembershipNo.isNotEmpty) {
           await _firestore
