    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snapshot) {
        if (snapshot.connectionState == ConnectionState.waiting) {
          return const Center(
            child: CircularProgressIndicator(color: Color(0xff1B2735)),
          );
        }
        ...
        if (snapshot.hasData && snapshot.data!.docs.isNotEmpty) {
          for (var doc in snapshot.data!.docs) {
            ...
            if (index != -1) {
              finalRates[index]['baseFare'] =
                  double.tryParse(dbData['baseFare'].toString()) ??
                  finalRates[index]['baseFare'];
              finalRates[index]['baseDistance'] =
                  double.tryParse(dbData['baseDistance'].toString()) ??
                  finalRates[index]['baseDistance'];
              finalRates[index]['perKm'] =
                  double.tryParse(dbData['perKm'].toString()) ??
                  finalRates[index]['perKm'];
              finalRates[index]['perMinute'] =
                  double.tryParse(dbData['perMinute'].toString()) ??
                  finalRates[index]['perMinute'];
              finalRates[index]['nightPct'] =
                  double.tryParse(dbData['nightFarePct'].toString()) ??
                  finalRates[index]['nightPct'];
              finalRates[index]['peakPct'] =
                  double.tryParse(dbData['peakFarePct'].toString()) ??
                  finalRates[index]['peakPct'];
            }
          }
        }
...
class _RateEditCardState extends State<RateEditCard> {
  ...
  void _initControllers() {
    _baseFareCtrl = TextEditingController(
      text: widget.rateData['baseFare'].toString(),
    );
    _baseDistCtrl = TextEditingController(
      text: widget.rateData['baseDistance'].toString(),
    );
    _perKmCtrl = TextEditingController(
      text: widget.rateData['perKm'].toString(),
    );
    _perMinCtrl = TextEditingController(
      text: widget.rateData['perMinute'].toString(),
    );
    _nightCtrl = TextEditingController(
      text: widget.rateData['nightPct'].toString(),
    );
    _peakCtrl = TextEditingController(
      text: widget.rateData['peakPct'].toString(),
    );
  }
          ...
          children: [
            ...
            Align(
              ...
              child: ElevatedButton.icon(
                ...
                onPressed: () async {
                  double baseFare =
                      double.tryParse(_baseFareCtrl.text) ??
                      widget.rateData['baseFare'];
                  double baseDistance =
                      double.tryParse(_baseDistCtrl.text) ??
                      widget.rateData['baseDistance'];
                  double perKm =
                      double.tryParse(_perKmCtrl.text) ??
                      widget.rateData['perKm'];
                  double perMinute =
                      double.tryParse(_perMinCtrl.text) ??
                      widget.rateData['perMinute'];
                  double nightFarePct =
                      double.tryParse(_nightCtrl.text) ??
                      widget.rateData['nightPct'];
                  double peakFarePct =
                      double.tryParse(_peakCtrl.text) ??
                      widget.rateData['peakPct'];
                  ...
                  try {
                    ...
                    if (context.mounted) {
                      ScaffoldMessenger.of(context).showSnackBar(
                        SnackBar(
                          content: Text(
                            "${widget.displayName} Rates Locked & Active in Cloud! 🚀",
                          ),
