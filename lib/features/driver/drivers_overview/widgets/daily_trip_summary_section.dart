...
class DailyTripSummarySection extends StatefulWidget {
  ...
  @override
  State<DailyTripSummarySection> createState() =>
...
class _DailyTripSummarySectionState extends State<DailyTripSummarySection> {
  ...
  String _selectedFilterType =
      'Today'; // 'Today', 'This Week', 'This Month', 'Custom Date'
  ...
  Future<void> _fetchDailySummary() async {
    ...
    } else if (_selectedFilterType == 'Custom Date' &&
    ...
    try {
      ...
      for (var doc in bookingsSnap.docs) {
        ...
        final status =
            (data['status']?.toString() ?? data['tripState']?.toString() ?? '')
                .toLowerCase();
        if (status == 'completed') {
          scheduledMoney +=
              double.tryParse(
                data['actualFare']?.toString() ??
      ...
      for (int i = 0; i < pickupFutures.length; i += 500) {
        int end = (i + 500 < pickupFutures.length)
            ? i + 500
            : pickupFutures.length;
    ...
    showDialog(
      ...
      builder: (ctx) {
        ...
        String vehicleFilter =
            'All Vehicles'; // Local state for vehicle category
        ...
        return StatefulBuilder(
          builder: (context, setDialogState) {
            ...
            // Filter the bookings
            List<Map<String, dynamic>> filteredBookings = _bookingsData.where((
              b,
            ) {
              // 1. Status Filter
              ...
              if (dialogFilter == 'All') {
                ...
                final status = (b['status']?.toString() ?? 'unknown')
                    .toLowerCase();
                final tState = (b['tripState']?.toString() ?? 'unknown')
                    .toLowerCase();
                        ...
                        children: vehicleTypes.map((f) {
                          ...
                          return Padding(
                            ...
                            child: ChoiceChip(
                              ...
                              onSelected: (val) {
                                if (val)
                                  setDialogState(() => vehicleFilter = f);
                              },
                            ),
                          );
                        }).toList(),
                          ...
                          : ListView.builder(
                              ...
                              itemBuilder: (context, index) {
                                ...
                                final driverMemNo =
                                    ...
                                    '';
                                final driverName =
                                    driverMemNo.isNotEmpty &&
                                        driverMemNo != baseDriverName
                                ...
                                else if (vcl.contains('van') ||
                                    vcl.contains('roof'))
                                  vehicleIcon = Icons.airport_shuttle;
                                else if (vcl.contains('car') ||
                                    vcl.contains('sedan') ||
                                    vcl.contains('mini'))
                                  vehicleIcon = Icons.directions_car;

                                Color statusColor = Colors.grey;
                                Color statusBg = Colors.grey.withValues(
                                  alpha: 0.1,
                                );
                                if (status == 'COMPLETED') {
                                  statusColor = Colors.green.shade700;
                                  statusBg = Colors.green.withValues(
                                    alpha: 0.1,
                                  );
                                  ...
                                  statusBg = Colors.purple.withValues(
                                    alpha: 0.1,
                                  );
                                  ...
                                  statusBg = Colors.orange.withValues(
                                    alpha: 0.1,
                                  );
                                }
  ...
  void _showRoadPickupsListDialog(BuildContext context) {
    showDialog(
      ...
      builder: (ctx) {
        ...
        return StatefulBuilder(
          builder: (context, setDialogState) {
            List<Map<String, dynamic>> filteredPickups = _roadPickupsData.where(
              (b) {
                if (dialogFilter == 'All') return true;
                final rawStatus =
                    (b['status']?.toString() ??
                            b['tripState']?.toString() ??
                            '')
                        .toLowerCase();
                final status = rawStatus.isEmpty ? 'completed' : rawStatus;
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    SingleChildScrollView(
                      scrollDirection: Axis.horizontal,
                      child: Row(
                        children: ['All', 'Completed', 'Ongoing'].map((f) {
                          final isSel = dialogFilter == f;
                          return Padding(
                            padding: const EdgeInsets.only(right: 8.0),
                            child: ChoiceChip(
                    ...
                    Expanded(
                      child: filteredPickups.isEmpty
                          ? const Center(
                              child: Text(
                                "No road pickups found for the selected filter",
                                style: TextStyle(color: Colors.grey),
                              ),
                            )
                          : ListView.separated(
  ...
  void _showTransactionsDialog(BuildContext context) {
    ...
    for (var b in _bookingsData) {
      final status =
          (b['status']?.toString() ?? b['tripState']?.toString() ?? '')
              .toLowerCase();
      if (status == 'completed') {
    ...
    for (var rp in _roadPickupsData) {
      final rawStatus =
          (rp['status']?.toString() ?? rp['tripState']?.toString() ?? '')
              .toLowerCase();
      final status = rawStatus.isEmpty ? 'completed' : rawStatus;
      if (status == 'completed') {
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    SingleChildScrollView(
                      scrollDirection: Axis.horizontal,
                      child: Row(
                        children: ['All', 'Scheduled', 'Road Pickups'].map((f) {
                                                    ...
                                                    children: [
                                                      ...
                                                      Row(
                                                        children: [
                                                          ...
                                                          Text(
                                                            'App Fee (3%): Rs ${commission.toStringAsFixed(1)}',
                                                            style:
                                                                const TextStyle(
                                                                  color: Color(
                                                                    0xFF10B981,
