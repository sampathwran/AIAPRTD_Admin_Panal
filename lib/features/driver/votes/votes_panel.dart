...
class _VotesPanelState extends State<VotesPanel> {
                  ...
                  onPressed: isSubmitting
                      ...
                      : () async {
                          ...
                          await FirebaseFirestore.instance.collection('polls').add({
                            ...
                            'votes':
                                {}, // map of docId -> {optionIndex, name, memberNo, phone}
                            'comments': [],
                          });
  ...
  Future<void> _generatePollPdf(
    Map<String, dynamic> data,
    String docId,
    List<int> optionCounts,
    double overallParticipation,
    int totalVotes,
  ) async {
      ...
      pw.MultiPage(
        ...
        build: (pw.Context context) {
          return [
            ...
            pw.SizedBox(height: 10),
            pw.Text(
              "Total Participation: $totalVotes votes (${overallParticipation.toStringAsFixed(1)}% of $_totalMembersCount members)",
              ...
              return pw.Container(
                ...
                child: pw.Row(
                  ...
                  children: [
                    ...
                    pw.Text(
                      "$count votes (${(pct * 100).toStringAsFixed(1)}%)",
                    ),
                  ],
                ),
              );
            ...
            if (votesMap.isEmpty)
              ...
              ...votesMap.entries.map((entry) {
                final voteData = entry.value;
                String selectedOption = options[voteData['optionIndex']];
                return pw.Container(
                  margin: const pw.EdgeInsets.only(bottom: 5),
                  child: pw.Text(
                    "- ${voteData['name']} (${voteData['memberNo']}) [Phone: ${voteData['phone']}] voted for: $selectedOption",
  ...
  void _showPollDetails(Map<String, dynamic> data, String docId) {
    ...
    double overallParticipation = _totalMembersCount > 0
        ? (totalVotes / _totalMembersCount) * 100
        : 0.0;
                    ...
                    ListView.builder(
                      ...
                      itemBuilder: (context, index) {
                        ...
                        String selectedOption =
                            options[voteData['optionIndex']];
                        return ListTile(
                          leading: const CircleAvatar(
                            child: Icon(Icons.person),
                          ),
                          title: Text(
                            "${voteData['name']} (${voteData['memberNo']})",
                          ),
                          subtitle: Text(
                ...
                itemBuilder: (context, index) {
                  ...
                  return Card(
                    ...
                    child: ListTile(
                      ...
                      subtitle: Text(
                        "Votes: $votesCount • Expires: ${expiresAt != null ? DateFormat('MMM dd, hh:mm a').format(expiresAt.toDate()) : 'Never'}",
                      ),
                      trailing: Chip(
                        label: Text(
