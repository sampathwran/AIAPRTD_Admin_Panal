...
import 'package:aiaprtd_admin_dashboard/features/driver/activation_requests/vehicle_photos_view.dart';
import 'package:aiaprtd_admin_dashboard/features/driver/activation_requests/document_review_view.dart';
      ...
      body: StreamBuilder<DocumentSnapshot>(
        ...
        builder: (context, snapshot) {
          ...
          if (currentCategory != null) {
            ...
            if (currentCategory == "14 Seater Van")
              currentCategory = "14 Seater";
          }
                    ...
                    onPressed: () async {
                      if (currentCategory == null) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          SnackBar(
                            content: const Text(
                              "Please select a pricing category first!",
                            ),
                      ...
                      if (success) {
                        ScaffoldMessenger.of(context).showSnackBar(
                          const SnackBar(
                            content: Text(
                              "Vehicle Request Approved & Activated Successfully!",
                            ),
    ...
    return Column(
      children: [
        Icon(
          icon,
          size: 16,
          color: const Color(0xff1B2735).withValues(alpha: 0.5),
        ),
      ...
      builder: (context, snapshot) {
        ...
        return Card(
          ...
          shape: RoundedRectangleBorder(
            ...
            side: BorderSide(
              color: const Color(0xff1B2735).withValues(alpha: 0.08),
            ),
          ),
          child: Padding(
...
class _EditDetailsDialogContent extends StatefulWidget {
  ...
  @override
  State<_EditDetailsDialogContent> createState() =>
      _EditDetailsDialogContentState();
}
...
class _EditDetailsDialogContentState extends State<_EditDetailsDialogContent> {
  ...
  void initState() {
    ...
    brandController = TextEditingController(
      text: widget.currentDetails['brand'] ?? '',
    );
    modelController = TextEditingController(
      text: widget.currentDetails['model'] ?? '',
    );
    yearController = TextEditingController(
      text: widget.currentDetails['year'] ?? '',
    );
  }
