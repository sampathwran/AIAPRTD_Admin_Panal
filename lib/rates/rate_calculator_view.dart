...
class _RateCalculatorViewState extends State<RateCalculatorView> {
  // Global Raw Inputs
  double pPrice = 445, dPrice = 407, lcFactor = 100, dailyExp = 1500, deadMileage = 20, commPct = 20;
  ...
  final List<Map<String, dynamic>> fleet = [
    {'id': 'budget', 'name': 'Budget (Alto/Nano)', 'eff': 20, 'lease': 8000, 'ins': 50000, 'maintenance': 10000, 'salary': 35, 'profitMargin': 33.55},
    {'id': 'mini', 'name': 'Mini (Axia/Vitz)', 'eff': 15, 'lease': 15000, 'ins': 80000, 'maintenance': 15000, 'salary': 40, 'profitMargin': 31.00},
    {'id': 'sedan', 'name': 'Sedan (Fit/Civic)', 'eff': 12, 'lease': 25000, 'ins': 150000, 'maintenance': 20000, 'salary': 45, 'profitMargin': 34.08},
    {'id': '6_seater', 'name': '6 Seater Van', 'eff': 10, 'lease': 20000, 'ins': 100000, 'maintenance': 25000, 'salary': 50, 'profitMargin': 29.50},
    {'id': '9_seater', 'name': '9 Seater Van', 'eff': 8.5, 'lease': 40000, 'ins': 180000, 'maintenance': 35000, 'salary': 60, 'profitMargin': 44.12},
    {'id': '14_seater', 'name': '14 Seater Van', 'eff': 6.5, 'lease': 50000, 'ins': 220000, 'maintenance': 45000, 'salary': 70, 'profitMargin': 45.31},
  ];
  ...
  Future<void> _loadFromFirebase() async {
    try {
      DocumentSnapshot doc = await FirebaseFirestore.instance.collection('admin_settings').doc('calculator').get();
      if (doc.exists) {
  ...
  Future<void> _saveToFirebase() async {
    try {
      await FirebaseFirestore.instance.collection('admin_settings').doc('calculator').set({
        'pPrice': pPrice,
        'dPrice': dPrice,
        'lcFactor': lcFactor,
        'dailyExp': dailyExp,
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(content: Text("Calculator Settings Saved to Cloud! ☁️"), backgroundColor: Colors.green),
        );
      }
    } catch (e) {
      ...
      child: Column(
        children: [
          ...
          Card(
            ...
            child: Padding(
              ...
              child: Wrap(
                ...
                children: [
                  _buildStableField("Petrol (Rs)", _petrolCtrl, (v) => setState(() => pPrice = v)),
                  _buildStableField("Diesel (Rs)", _dieselCtrl, (v) => setState(() => dPrice = v)),
                  _buildStableField("Life Cost %", _lcFactorCtrl, (v) => setState(() => lcFactor = v)),
                  _buildStableField("Daily Exp", _dailyExpCtrl, (v) => setState(() => dailyExp = v)),
                  _buildStableField("Dead Mile %", _deadMileCtrl, (v) => setState(() => deadMileage = v)),
                  _buildStableField("Comm %", _commPctCtrl, (v) => setState(() => commPct = v)),
                ],
              ),
            ),
          ),
          ...
          ElevatedButton.icon(
            ...
            label: const Text("Save Calculator Config to Cloud", style: TextStyle(fontWeight: FontWeight.bold)),
  ...
  Widget _buildVehicleCard(Map<String, dynamic> v) {
    double fuel = ((v['id'] == '9_seater' || v['id'] == '14_seater' ? dPrice : pPrice) / v['eff']);
    ...
    return Card(
      ...
      child: ExpansionTile(
        ...
        subtitle: Text("මගියාගෙන් අය කරන ගාස්තුව: රු. ${finalFare.round()}/km", style: const TextStyle(color: Colors.blue, fontWeight: FontWeight.bold, fontSize: 13)),
        children: [
          ...
          _infoRow("රියදුරුගේ ලාභය (Net Profit)", netProfit, isBold: true, color: Colors.green),
          const Divider(thickness: 1.5, indent: 16, endIndent: 16),
          _infoRow("මගියාගෙන් අය කළ යුතු මුදල", finalFare, isBold: true, color: Colors.blue),
  ...
  Widget _infoRow(String label, double val, {bool isBold = false, Color? color}) {
    return Padding(
      ...
      child: Row(
        ...
        children: [
          ...
          Text("රු. ${val.toStringAsFixed(2)}", style: TextStyle(fontWeight: isBold ? FontWeight.bold : FontWeight.normal, color: color, fontSize: 13)),
        ],
      ),
    );
  }

  Widget _buildStableField(String label, TextEditingController controller, Function(double) onChanged) {
    return SizedBox(
      width: 102,
      height: 42,
      child: TextField(
    ...
    return Column(
      children: [
        ...
        Row(
          children: [
            Icon(
              percentage == 0 ? Icons.remove_rounded : (isUp ? Icons.trending_up_rounded : Icons.trending_down_rounded),
              color: percentage == 0 ? Colors.grey : (isUp ? Colors.red : Colors.green),
              size: 16,
            ),
