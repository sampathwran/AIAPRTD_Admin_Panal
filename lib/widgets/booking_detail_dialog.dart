...
class BookingDetailDialog extends StatelessWidget {
  ...
  Widget _buildDialogContent(BuildContext context, Map<String, dynamic> data) {
    ...
    final String? driverMemberId =
        (data['acceptedBy'] ?? data['driverId'])?.toString();
    ...
    debugPrint('📦 BookingDetailDialog: status=$status driverId=$driverMemberId '
...
class _OngoingPickupMapState extends State<_OngoingPickupMap> {
  ...
  Future<void> _loadDriverIcon() async {
    final String vc = (widget.data['vehicleCategory'] ??
            (widget.data['vehicle'] != null ? widget.data['vehicle']['name'] : 'mini'))
        .toString()
        .toLowerCase();
        
    String assetPath = 'assets/vehicle_icons/mini_marker.png';
    if (vc.contains('budget')) assetPath = 'assets/vehicle_icons/budget_marker.png';
    else if (vc.contains('sedan')) assetPath = 'assets/vehicle_icons/sedan_marker.png';
    else if (vc.contains('6')) assetPath = 'assets/vehicle_icons/mini_van_marker.png';
    else if (vc.contains('9')) assetPath = 'assets/vehicle_icons/flatRoof_marker.png';
    else if (vc.contains('14')) assetPath = 'assets/vehicle_icons/highRoof_marker.png';
  ...
  Future<void> _fetchRoute(LatLng origin) async {
    ...
    final targetUrl = 'https://maps.googleapis.com/maps/api/directions/json'
        '?origin=${origin.latitude},${origin.longitude}'
        '&destination=${_targetPoint.latitude},${_targetPoint.longitude}'
        '&mode=driving'
        '&key=$_apiKey';

    final url = Uri.parse('https://api.allorigins.win/raw?url=' + Uri.encodeComponent(targetUrl));
  ...
  Widget build(BuildContext context) {
    ...
    try {
      final members =
          Provider.of<MemberProvider>(context, listen: false).allMembersList;
    ...
    return StreamBuilder<QuerySnapshot>(
      ...
      builder: (context, snap) {
        ...
        final LatLng initialPos = driverPos ??
          ...
          children: [
            ...
            Positioned(
              ...
              child: _EtaCard(
                ...
                acceptedTime: widget.data['acceptedAt']?.toString() ?? widget.data['acceptedTime']?.toString(),
              ),
            ),
...
class _HistoricalMapViewState extends State<_HistoricalMapView> {
    ...
    try {
      ...
      if (res.statusCode == 200) {
        ...
        if (body['status'] == 'OK') {
          final routes = body['routes'] as List;
          if (routes.isNotEmpty) {
            final polylineString = routes[0]['overview_polyline']['points'] as String;
  ...
  String _timeTaken() {
    ...
    final arrivedTime = widget.data['arrivedAt'] ??
  ...
  Widget build(BuildContext context) {
    ...
    if (widget.pickupPoint != null && widget.dropPoint != null) {
      polylines.add(Polyline(
        ...
        points: _routePoints.isNotEmpty ? _routePoints : [widget.pickupPoint!, widget.dropPoint!],
        color: Colors.blue.shade700,
        width: 5,
        jointType: JointType.round,
        startCap: Cap.roundCap,
            ...
            Future.delayed(const Duration(milliseconds: 500), () {
              if (widget.pickupPoint != null && widget.dropPoint != null) {
                _ctrl?.animateCamera(CameraUpdate.newLatLngBounds(
                  LatLngBounds(
                    southwest: LatLng(
                      min(widget.pickupPoint!.latitude,
                          widget.dropPoint!.latitude),
                      min(widget.pickupPoint!.longitude,
                          widget.dropPoint!.longitude),
                    ),
                    northeast: LatLng(
                      max(widget.pickupPoint!.latitude,
                          widget.dropPoint!.latitude),
                      max(widget.pickupPoint!.longitude,
                          widget.dropPoint!.longitude),
                    ),
                  ),
                  60,
