...
class ChangeBankDetailsProvider with ChangeNotifier {
  ...
  // =========================================================================
  Future<bool> approveBankRequest(
    String membershipNo,
    ...
    try {
      ...
      var memberSnapshot = await firestore
          .collection('member')
      ...
      batch.set(
        firestore.collection('payments').doc('${membershipNo}_bank'),
        {
    ...
    try {
      ...
      for (var doc in memberSnapshot.docs) {
        await doc.reference.update({
          ...
          'bankRejectReason':
              reason, // 👈 Member app එකට පෙන්නන්න මේක පාවිච්චි කරන්න පුළුවන්
        });
      }
