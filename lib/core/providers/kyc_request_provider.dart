...
class KycRequestProvider with ChangeNotifier {
  ...
  // =========================================================================
  List<Map<String, dynamic>> get pendingRequests => _allKycRequests
      .where((k) => k['kycApprovalStatus'] == 'pending')
      .toList();

  List<Map<String, dynamic>> get approvedRequests => _allKycRequests
      .where((k) => k['kycApprovalStatus'] == 'approved')
      .toList();

  List<Map<String, dynamic>> get rejectedRequests => _allKycRequests
      .where((k) => k['kycApprovalStatus'] == 'rejected')
      .toList();
  ...
  void startListeningToKycRequests() {
    ...
    _kycSubscription = firestore
        .collection('verify_kyc')
        .snapshots()
        .listen(
          (querySnapshot) {
            debugPrint(
              "🔥 KYC PROVIDER: ඩේටාබේස් එකෙන් ලයිව් සිග්නල් එකක් ආවා!",
            );
            ...
            _allKycRequests = querySnapshot.docs.map((doc) {
              ...
              data['faceVerificationUrl'] =
                  data['faceVerificationUrl']?.toString() ?? '';

              // 🟢 ස්ටේටස්
              data['kycApprovalStatus'] =
                  data['kycApprovalStatus']?.toString() ?? 'pending';

              // 🕒 වෙලාව හැන්ඩල් කිරීම
              if (data['submittedAt'] != null &&
                  data['submittedAt'] is Timestamp) {
                data['submittedAt'] = (data['submittedAt'] as Timestamp)
                    .toDate()
                    .toString();
              } else {
