          ...
          children: [
            ...
            Expanded(
              child: memberProvider.isLoading
                  ...
                  ? const Center(
                child: Text('No active drivers matching search query.', style: TextStyle(color: Colors.grey, fontSize: 13)),
              )
                  : Container(
                    ...
                    child: Scrollbar(
                      ...
                      child: SingleChildScrollView(
                        ...
                        child: ConstrainedBox(
                          ...
                          child: DataTable(
                            headingRowColor: WidgetStateProperty.all(Colors.indigo.withValues(alpha: 0.02)),
                            headingRowHeight: 40,
                            dataRowMinHeight: 44,
                            dataRowMaxHeight: 44,
                            horizontalMargin: 12,
                            columnSpacing: 24,
                            columns: const [
                              DataColumn(label: Text('ID', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: Colors.indigo))),
                              DataColumn(label: Text('Full Name', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: Colors.indigo))),
                              DataColumn(label: Text('Mobile', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: Colors.indigo))),
                              DataColumn(label: Text('Vehicle Number', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: Colors.indigo))),
                              DataColumn(label: Text('Vehicle Type', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: Colors.indigo))),
                              DataColumn(label: Text('Rating', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: Colors.indigo))),
                              DataColumn(label: Text('Status', style: TextStyle(fontWeight: FontWeight.bold, fontSize: 12, color: Colors.indigo))),
                            ],
                            rows: filteredMembers.map((driver) {
                              return DataRow(cells: [
                                DataCell(Text(driver['membershipNo'] ?? '-', style: const TextStyle(fontWeight: FontWeight.w600, fontSize: 12))),
                                DataCell(Text(driver['fullName'] ?? 'Unknown', style: const TextStyle(fontSize: 12))),
                                DataCell(Text(driver['mobile'] ?? '-', style: const TextStyle(fontSize: 12))),
                                DataCell(Text(driver['vehicleNumber'] ?? '-', style: const TextStyle(fontWeight: FontWeight.bold, fontSize: 12))),
                                DataCell(Text(driver['vehicleType'] ?? '-', style: const TextStyle(fontSize: 12))),
                                DataCell(Text('⭐ ${driver['rating'] ?? '5.0'}', style: const TextStyle(fontSize: 12))),
                                DataCell(Container(
