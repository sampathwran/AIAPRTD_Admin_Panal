...
import 'scheduled_bookings/pending_tab.dart';
import 'scheduled_bookings/ongoing_pickup_tab.dart';
import 'scheduled_bookings/ongoing_drop_tab.dart';
import 'scheduled_bookings/completed_tab.dart';
import 'scheduled_bookings/cancelled_tab.dart';
...
class ScheduledBookingsFullPanel extends StatefulWidget {
  ...
  @override
  State<ScheduledBookingsFullPanel> createState() => _ScheduledBookingsFullPanelState();
}

class _ScheduledBookingsFullPanelState extends State<ScheduledBookingsFullPanel> {
                ...
                children: [
                  ...
                  const Column(
                    ...
                    children: [
                      ...
                      SizedBox(height: 4),
                      Text('Manage and monitor all scheduled bookings', style: TextStyle(color: Colors.grey, fontSize: 13)),
                    ],
                  ),
                ],
              ),
  ...
  Widget _buildMainContent(List<QueryDocumentSnapshot> allDocs) {
    ...
    for (var doc in allDocs) {
      ...
      String vc = data['vehicleCategory']?.toString() ?? (data['vehicle'] != null ? data['vehicle']['name']?.toString() : null) ?? 'Mini';
    ...
    String selectedChipLabel = displayCategories.firstWhere(
      (c) => c.startsWith(_selectedCategory == 'All' ? 'All' : _selectedCategory), 
    ...
    final ongoingPickupDocs = categoryFilteredDocs.where((doc) {
      ...
      final hasStarted = data['startedAt'] != null || data['tripStartTime'] != null;
      return (status == 'ongoing pickup' && !hasStarted) || (status == 'accepted' && !hasStarted);
    }).toList();
    ...
    final ongoingDropDocs = categoryFilteredDocs.where((doc) {
      ...
      final status = data['status']?.toString().toLowerCase() ?? '';
      final hasStarted = data['startedAt'] != null || data['tripStartTime'] != null;
      return status == 'ongoing drop' || status == 'arrived' || status == 'ongoing' || ((status == 'accepted' || status == 'ongoing pickup') && hasStarted);
    }).toList();
  ...
  Widget _buildEmptyStateForAllTabs() {
    return const Column(
      children: [
         SizedBox(height: 40),
         AdminEmptyBookingState(title: "No Data", subtitle: "There are no bookings available."),
      ],
    );
  }
