...
class MarketplaceCategoriesTab extends StatefulWidget {
  ...
  @override
  State<MarketplaceCategoriesTab> createState() => _MarketplaceCategoriesTabState();
}
...
class _MarketplaceCategoriesTabState extends State<MarketplaceCategoriesTab> {
  ...
  Future<void> _addCategory() async {
    ...
    try {
      final ref = FirebaseStorage.instance.ref().child('marketplace_categories').child('${DateTime.now().millisecondsSinceEpoch}.jpg');
      await ref.putData(await _catImageFile!.readAsBytes());
      final imageUrl = await ref.getDownloadURL();

      await FirebaseFirestore.instance.collection('marketplace_categories').add({
  ...
  void _deleteCategory(String id) {
    FirebaseFirestore.instance.collection('marketplace_categories').doc(id).delete();
  }

  void _manageSubcategories(BuildContext context, String catId, List<dynamic> existingSubs) {
    showDialog(
      context: context,
      builder: (ctx) {
        return SubcategoriesManagerDialog(catId: catId, subcategories: existingSubs);
      },
    );
  }
        ...
        children: [
                ...
                children: [
                  GestureDetector(
                    ...
                    child: Container(
                      ...
                      child: _catImageFile != null
                          ? (kIsWeb ? Image.network(_catImageFile!.path, fit: BoxFit.cover) : Image.file(File(_catImageFile!.path), fit: BoxFit.cover))
                  ...
                  Expanded(
                    child: TextField(
                      ...
                      decoration: const InputDecoration(labelText: "Main Category Name", border: OutlineInputBorder()),
                    ),
                  ),
                  const SizedBox(width: 16),
                  ElevatedButton(
                    onPressed: _isLoading ? null : _addCategory,
                    child: _isLoading ? const CircularProgressIndicator() : const Text("Add Category"),
                  )
                ],
              ),
            ),
          ...
          Expanded(
            child: StreamBuilder<QuerySnapshot>(
              stream: FirebaseFirestore.instance.collection('marketplace_categories').snapshots(),
                        ...
                        trailing: Row(
                          ...
                          children: [
                            IconButton(
                              ...
                              onPressed: () => _manageSubcategories(context, docs[index].id, subs),
                            ),
                            IconButton(
...
class SubcategoriesManagerDialog extends StatefulWidget {
  ...
  const SubcategoriesManagerDialog({super.key, required this.catId, required this.subcategories});

  @override
  State<SubcategoriesManagerDialog> createState() => _SubcategoriesManagerDialogState();
}

class _SubcategoriesManagerDialogState extends State<SubcategoriesManagerDialog> {
  ...
  Future<void> _addSubcategory() async {
    ...
    setState(() => _isLoading = true);
    try {
      final ref = FirebaseStorage.instance.ref().child('marketplace_subcategories').child('${DateTime.now().millisecondsSinceEpoch}.jpg');
      ...
      await FirebaseFirestore.instance.collection('marketplace_categories').doc(widget.catId).update({
          ...
          children: [
            Row(
              children: [
                GestureDetector(
                  ...
                  child: Container(
                    ...
                    child: _subIconFile != null
                        ? (kIsWeb ? Image.network(_subIconFile!.path, fit: BoxFit.cover) : Image.file(File(_subIconFile!.path), fit: BoxFit.cover))
                ...
                IconButton(
                  icon: _isLoading ? const CircularProgressIndicator() : const Icon(Icons.add, color: Colors.blue),
