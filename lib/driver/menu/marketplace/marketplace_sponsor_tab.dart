...
class _MarketplaceSponsorTabState extends State<MarketplaceSponsorTab> {
  ...
  Future<void> _uploadSponsorAd() async {
    ...
    try {
      ...
      final ref = FirebaseStorage.instance.ref().child('marketplace_sponsor_ads').child('${DateTime.now().millisecondsSinceEpoch}.$ext');
      await ref.putData(await _mediaFile!.readAsBytes());
      final url = await ref.getDownloadURL();

      await FirebaseFirestore.instance.collection('marketplace_sponsor_ads').add({
        'type': _mediaType,
        'mediaUrl': url,
  ...
  void _deleteAd(String id) {
    FirebaseFirestore.instance.collection('marketplace_sponsor_ads').doc(id).delete();
  }
        ...
        children: [
                ...
                children: [
                  Container(
                    ...
                    child: _mediaFile != null 
                        ? (_mediaType == 'image' ? (kIsWeb ? Image.network(_mediaFile!.path, fit: BoxFit.cover) : Image.file(File(_mediaFile!.path), fit: BoxFit.cover)) : const Center(child: Icon(Icons.videocam, size: 40)))
                  ...
                  ElevatedButton(
                    ...
                    child: _isLoading ? const CircularProgressIndicator() : const Text("Upload Sponsor Ad"),
                  )
                ],
              ),
            ),
          ...
          Expanded(
            child: StreamBuilder<QuerySnapshot>(
              stream: FirebaseFirestore.instance.collection('marketplace_sponsor_ads').orderBy('createdAt', descending: true).snapshots(),
              builder: (context, snapshot) {
                if (!snapshot.hasData) return const Center(child: CircularProgressIndicator());
                final docs = snapshot.data!.docs;
                if (docs.isEmpty) return const Center(child: Text("No sponsor ads found."));
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return Card(
                      ...
                      child: Stack(
                        ...
                        children: [
                          isVideo 
                            ? Container(color: Colors.black, child: const Center(child: Icon(Icons.play_circle_fill, size: 50, color: Colors.white)))
